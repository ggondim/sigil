...
function defaults() {
  return {
    ...
    // remote), 'path' (legacy, key on absolute root path), or 'explicit'
    // (committed marker only, no path fallback). See config.js `project`.
    project: { identity: 'remote' },
    // Read-time pod-kind visibility enforcement (P2): 'device' (owner-scope
    // private facts to the creating device) or 'off'. See config.js `privacy`.
