...
export function registerRemember(registry) {
  registry.register('remember', async (params) => {
    ...
    for (const item of facts) {
      ...
      if (!text) continue;
      // P13: classificação vinda do cliente (ex.: claude -p local) — respeitada no
      // caminho LLM-less via factOverrides.
      const factOverrides = (item && typeof item === 'object')
        ? { category: item.category, importance: item.importance }
        : null;
      const result = await ingestDocument({ content: text, namespace, classify: true, podUids, factOverrides });
