...
// triggers the judge. config.json is the source of truth now, so we seed the
// thresholds via the test seam (before any config read) instead of env vars.
import { __setTestConfig } from '../../src/setup/config-store.js';

__setTestConfig({
  memory: {
    skipThreshold: 0.999,
    ambiguousThreshold: 0.40,
